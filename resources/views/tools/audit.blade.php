...
>
    ...
    <section class="relative">
        ...
        <div class="mx-auto max-w-4xl px-6 pb-16 pt-16 text-center lg:px-8 lg:pb-24 lg:pt-24">
            ...
            <h1 class="mx-auto mt-6 max-w-2xl text-balance text-4xl font-semibold tracking-tight text-slate-900 sm:text-5xl">
                Free SEO audit for any page
            </h1>
            <p class="mx-auto mt-5 max-w-xl text-balance text-[17px] leading-8 text-slate-600">
                On-page fixes, keyword targeting, Core Web Vitals and a top-ranking competitor benchmark — in about a minute.
            </p>
            ...
            <div class="relative mx-auto mt-10 max-w-3xl">
                ...
                <form id="ga-form" class="text-left" data-action="{{ route('guest-audit.store') }}" novalidate>
                    <div class="flex flex-col rounded-[20px] bg-white p-2 shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)] ring-1 ring-slate-200/80 transition focus-within:ring-2 focus-within:ring-indigo-500/70 sm:flex-row sm:items-center sm:divide-x sm:divide-slate-200/70 divide-y divide-slate-100 sm:divide-y-0">
                        <div class="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <input id="ga-url" name="url" type="text" inputmode="url" autocomplete="url" autofocus required placeholder="yourwebsite.com/page"
                        ...
                        <div class="flex items-center gap-3 px-3 py-2.5 sm:w-52">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <input id="ga-keyword" name="keyword" type="text" required maxlength="200" placeholder="best seo tools"
                                    class="w-full border-0 bg-transparent p-0 text-[15px] font-medium text-slate-900 placeholder:font-normal placeholder:text-slate-400 focus:outline-none focus:ring-0">
                            </div>
                        </div>
                        <div class="flex items-center gap-3 px-3 py-2.5 sm:w-44">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <select id="ga-country" name="country" class="-ml-0.5 w-full border-0 bg-transparent p-0 text-[15px] font-medium text-slate-900 focus:outline-none focus:ring-0">
                    ...
                    <p class="mt-5 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs text-slate-500">
                        ...
                        <span class="text-slate-300">·</span><span>No signup for your first audit</span>
                ...
                <div id="ga-success" class="hidden rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-[0_30px_70px_-28px_rgba(15,23,42,0.30)]">
                    ...
                    <a href="{{ route('register') }}" class="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800">Create a free account for unlimited audits →</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <div id="ga-email-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="ga-email-title" class="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-900/5">
            <div class="px-7 pt-7">
                ...
                <p id="ga-email-modal-msg" class="mt-2 text-sm leading-6 text-slate-600">This one’s on us — tell us where to send your report and we’ll deliver it in about a minute.</p>
            </div>
            <form id="ga-email-form" class="px-7 pb-7 pt-5" novalidate>
                ...
                <div class="mt-5 flex flex-col gap-2 sm:flex-row-reverse">
                    <button type="submit" id="ga-email-submit"
                        ...
                        <svg id="ga-email-spinner" class="hidden h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                        <span id="ga-email-label">Email me my audit</span>
                    </button>
                    <button type="button" id="ga-email-cancel" class="rounded-xl px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <section class="border-t border-slate-200 bg-slate-50">
        <div class="mx-auto max-w-5xl px-6 py-16 lg:px-8">
            <h2 class="text-center text-2xl font-semibold tracking-tight text-slate-900">What’s in your free audit</h2>
            <div class="mt-10 grid gap-6 sm:grid-cols-3">
                @foreach ([
                    ['On-page SEO', 'Titles, meta, headings, images, links and crawlability — every issue turned into a prioritized fix.'],
                    ['Keyword analysis', 'How well the page targets your keyword, plus where you sit against the top-ranking competitors.'],
                    ['Core Web Vitals', 'Mobile & desktop performance signals so you fix speed and rankings together.'],
                ] as $f)
    ...
    <script>
        (function () {
            ...
            function setLoading(on) { btn.disabled = on; label.textContent = on ? 'Auditing your page…' : 'Run free audit'; spinner.classList.toggle('hidden', !on); arrow.classList.toggle('hidden', on); }
            function setEmailLoading(on) { if (emailSubmit) emailSubmit.disabled = on; if (emailSpinner) emailSpinner.classList.toggle('hidden', !on); if (emailLabel) emailLabel.textContent = on ? 'Sending…' : 'Email me my audit'; }
