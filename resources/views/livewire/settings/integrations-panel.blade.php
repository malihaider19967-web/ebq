    ...
    <div class="px-6 py-5">
        @if ($googleAccount)
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('google.redirect') }}" class="rounded-md border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Reconnect</a>
            </div>
        @else
            <a href="{{ route('google.redirect') }}" class="inline-flex items-center gap-1.5 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-700">
