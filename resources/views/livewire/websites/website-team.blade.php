            ...
            @if (! $readonly)
                <form wire:submit="inviteMember" class="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end">
                    <div class="min-w-0 flex-1">
                        <label for="invite-email-{{ $websiteId }}" class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">Email address</label>
                    ...
                    <button type="submit" class="inline-flex shrink-0 items-center justify-center rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700">
                        Send invite
                    </button>
                </form>
            @endif
