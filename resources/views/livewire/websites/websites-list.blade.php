<div class="space-y-6">
    ...
    @if ($ownedWebsites->isNotEmpty() || $sharedWebsites->isNotEmpty())
        ...
        @if ($sharedWebsites->isNotEmpty())
            <div class="@if($ownedWebsites->isNotEmpty()) mt-10 @endif">
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($sharedWebsites as $site)
                        <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                            <div class="flex items-center gap-3">
                                <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-50 text-sm font-bold text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">
                                    {{ strtoupper(substr($site->domain, 0, 1)) }}
                                </div>
                                <div>
                                    <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $site->domain }}</h3>
                            ...
                            <p class="mt-3 text-xs text-slate-500 dark:text-slate-400">Owner: {{ $site->user->name }} ({{ $site->user->email }})</p>
                        </div>
                    @endforeach
                </div>
            </div>
