<div class="flex w-full flex-col gap-3 sm:w-auto">
    ...
    <div class="flex items-center justify-end gap-2">
        <a href="{{ route('reports.index') }}"
            class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
        ...
        <button type="button" wire:click="sendReport" wire:loading.attr="disabled"
            class="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-indigo-500 dark:hover:bg-indigo-400">
    ...
    {{-- Sync timestamps --}}
    @if ($website)
        <div class="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 text-[11px] text-slate-400 dark:text-slate-500">
            <span>
