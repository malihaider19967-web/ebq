...
import { EmptyState, Spinner, Button } from './primitives';
...
/**
 * "From Google Search Console" — top ranking queries for this post URL.
 * Click a row to set it as the focus keyphrase.
 */
export default function GscSuggestions({ postId, onChoose }) {
	const [state, setState] = useState(() => {
		const cached = cache.get(postId);
		return cached
			? { loading: false, suggestions: cached.suggestions, diagnostic: cached.diagnostic, debug: cached.debug }
			: { loading: true, suggestions: [], diagnostic: null, debug: null };
	});
		...
		<div className="ebq-stack">
			<div className="ebq-row ebq-row--between">
				<span className="ebq-text-xs ebq-text-soft" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
					<IconChart /> {__('Top queries this URL is ranking for', 'ebq-seo')}
				</span>
				<button
					type="button"
					...
					aria-label={__('Refresh Search Console suggestions', 'ebq-seo')}
				>
					<IconRefresh />
				</button>
			</div>
